...
/**
 * Updates forum data, returns number of affected rows on success and false otherwise
 *
 * @param $parameters
 * 
 ...
 *
 */
function updateForums($parameters){
	...
	foreach($parameters['values'] as $key => $val){
		if(array_key_exists($key, $forumColumnDescription)){
			$sql .= $separator.$key.' = ?';
			$separator = ', ';
